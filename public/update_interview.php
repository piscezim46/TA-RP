...
if ($stmt = $conn->prepare($sql)) {
  ...
  if ($stmt->execute()) {
    $stmt->close();
    $sel = $conn->prepare("SELECT i.id, i.applicant_id, i.position_id, i.interview_datetime, i.status_id, COALESCE(s.name,'') AS status_name, COALESCE(s.status_color,'') AS status_color, i.location, i.result, i.comments, i.created_at, i.updated_at, i.created_by, COALESCE(u.name, '') AS created_by_name, COALESCE(d.department_name, '') AS created_by_department FROM interviews i LEFT JOIN interview_statuses s ON i.status_id = s.id LEFT JOIN users u ON i.created_by = u.id LEFT JOIN departments d ON u.department_id = d.department_id WHERE i.id = ? LIMIT 1");
    if ($sel) {
      $sel->bind_param('i', $id);
      $sel->execute();
      $res = $sel->get_result();
