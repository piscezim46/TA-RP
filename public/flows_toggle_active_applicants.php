...
// update applicants_status and all transitions that reference the status
$conn->begin_transaction();
try {
    $stmt = $conn->prepare('UPDATE applicants_status SET active = ? WHERE status_id = ?');
    ...
    $stmt2 = $conn->prepare('UPDATE applicants_status_transitions SET active = ?, updated_by = ? WHERE from_status_id = ? OR to_status_id = ?');
    if (!$stmt2) throw new Exception('Prepare failed: ' . $conn->error);
    $stmt2->bind_param('iiii', $new_active, $updated_by, $status_id, $status_id);
    ...
    $conn->rollback();
    error_log('flows_toggle_active_applicants.php error: ' . $e->getMessage());
