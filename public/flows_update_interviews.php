<?php
// public/flows_update_interviews.php - update one or more existing interview statuses and their transitions
session_start();
...
try {
    ...
    foreach ($rows as $dataRow) {
        ...
        // Allow empty transitions (terminal statuses)

        // check sort conflict with other active statuses
        $stmt = $conn->prepare('SELECT id FROM interview_statuses WHERE active = 1 AND sort_order = ? AND id <> ? LIMIT 1');
        ...
        // perform update
        $up = $conn->prepare('UPDATE interview_statuses SET name=?, status_color=?, pool_id=?, active=?, sort_order=? WHERE id=?');
        ...
        // replace transitions
        $del = $conn->prepare('DELETE FROM interviews_status_transitions WHERE from_status_id = ?');
        if (!$del) throw new Exception('Prepare delete failed');
        $del->bind_param('i',$id);
        if (!$del->execute()) throw new Exception('Delete failed: ' . $del->error);
        $del->close();

        // include audit info on transition inserts (use numeric user id)
        $updated_by = isset($_SESSION['user']['id']) ? (int)$_SESSION['user']['id'] : 0;
        if (is_array($transitions) && count($transitions)) {
            $ins = $conn->prepare('INSERT INTO interviews_status_transitions (from_status_id, to_status_id, active, updated_by, created_at) VALUES (?, ?, 1, ?, NOW())');
            if (!$ins) throw new Exception('Prepare insert trans failed');
            foreach($transitions as $to) {
                $to_id = (int)$to;
                $ins->bind_param('iii',$id,$to_id,$updated_by);
    ...
    $conn->rollback();
    error_log('flows_update_interviews.php error: '.$e->getMessage());
