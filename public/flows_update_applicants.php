<?php
// public/flows_update_applicants.php - update one or more existing applicant statuses and their transitions
session_start();
...
try {
    ...
    foreach ($rows as $dataRow) {
        ...
        $stmt = $conn->prepare('SELECT status_id FROM applicants_status WHERE active = 1 AND sort_order = ? AND status_id <> ? LIMIT 1');
        ...
        // perform update
        $up = $conn->prepare('UPDATE applicants_status SET status_name=?, status_color=?, pool_id=?, active=?, sort_order=? WHERE status_id=?');
        ...
        // replace transitions
        $del = $conn->prepare('DELETE FROM applicants_status_transitions WHERE from_status_id = ?');
        ...
        $ins = $conn->prepare('INSERT INTO applicants_status_transitions (from_status_id, to_status_id, active, updated_by, created_at) VALUES (?, ?, 1, ?, NOW())');
    ...
    $conn->rollback();
    error_log('flows_update_applicants.php error: '.$e->getMessage());
